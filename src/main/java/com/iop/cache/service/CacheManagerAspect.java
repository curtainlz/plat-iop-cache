package com.iop.cache.service;

import com.iop.cache.annotation.CacheDelete;
import com.iop.cache.annotation.CachePut;
import com.iop.cache.annotation.Cacheable;
import com.iop.cache.config.CacheConfigProperties;
import com.iop.cache.service.cache.CacheService;
import com.iop.cache.utils.CacheUtil;
import com.iop.cache.utils.KeyGenerators;
import com.iop.cache.utils.SpringExpressionParserUtil;
...
@Component
@Slf4j
@ConditionalOnProperty(name = "iop.cache.enable", havingValue = "true")
public class CacheManagerAspect {
    ...
    @Pointcut("execution(* com.iop..*.*(..)) && @annotation(com.iop.cache.annotation.Cacheable)")
    public void executionOfCacheableMethod() {
    }

    @Pointcut("execution(* com.iop..*.*(..)) && @annotation(com.iop.cache.annotation.CachePut)")
    public void executionOfCachePutMethod() {
    }

    @Pointcut("execution(* com.iop..*.*(..)) && @annotation(com.iop.cache.annotation.CacheDelete)")
    ...
    public void putInCache(final JoinPoint joinPoint, final Object returnObject) {
        ...
        try {
            ...
            if (!cacheConfigProperties.isEnableCache()) return;

            CachePut cachePutAnnotation = getAnnotation(joinPoint, CachePut.class);
            Object cacheKey = springExpressionParserUtil.parseAndGetCacheKeyFromExpression(cachePutAnnotation.keyExpression(), returnObject, joinPoint.getArgs(), cachePutAnnotation.keyGenerator());
    ...
    public void deleteCache(final JoinPoint joinPoint, final Object returnObject) {
        ...
        try {
            ...
            if (!cacheDeleteAnnotation.removeAll()) {
                cacheKey = springExpressionParserUtil.parseAndGetCacheKeyFromExpression(cacheDeleteAnnotation.keyExpression(), returnObject, joinPoint.getArgs(), cacheDeleteAnnotation.keyGenerator());
            }
    ...
    public Object getAndSaveInCache(final ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        if (!cacheConfigProperties.isEnableCache()) return callActualMethod(proceedingJoinPoint);
        ...
        try {
            ...
            if (StringUtils.hasText(cacheableAnnotation.keyExpression())) {
                cacheKey = CacheUtil.buildCacheKey(proceedingJoinPoint.getArgs());
            } else {
                cacheKey = springExpressionParserUtil.parseAndGetCacheKeyFromExpression(cacheableAnnotation.keyExpression(), null, proceedingJoinPoint.getArgs(), keyGenerator);
            }
            ...
            if (returnObject != null) {
                try {
                    if (cacheableAnnotation.isAsync()) {
                        redisCacheService.saveInRedisAsync(new String[]{cacheableAnnotation.cacheName()}, cacheKey, returnObject, cacheableAnnotation.TTL());
                    } else {
                        redisCacheService.save(new String[]{cacheableAnnotation.cacheName()}, cacheKey, returnObject, cacheableAnnotation.TTL());
                    }
                } catch (Exception e) {
                    log.error("getAndSaveInCache # Exception occurred while trying to save data in redis##" + e.getMessage(), e);
                }
            }
        }
        return returnObject;
    ...
    private <T extends Annotation> T getAnnotation(JoinPoint proceedingJoinPoint, Class<T> annotationClass) throws NoSuchMethodException {
