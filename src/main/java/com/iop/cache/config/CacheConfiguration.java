...
import com.iop.cache.service.cache.*;
import com.iop.cache.service.listener.CacheMessageListener;
import com.iop.cache.utils.SpringUtils;
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                ...
                try {
                    if (!redisExecutor.awaitTermination(awaitTerminationSeconds, TimeUnit.SECONDS)) {
                        log.info("Redisson Pool Executor did not terminate in the specified time.");
                        List<Runnable> droppedTasks = redisExecutor.shutdownNow();
                        log.info("Redisson Pool Executor was abruptly shutdown. " + droppedTasks.size() + " tasks will not be executed."); //optional **
                    }
                } catch (InterruptedException e) {
                    log.error("Redisson Pool Executor shutdown Error: " + e.getMessage(), e);
                }
            }));
        }
    }
